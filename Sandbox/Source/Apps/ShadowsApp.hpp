...
public:
	...
	void on_init() override
	{
		...
		l.dirLight.direction = { -0.93f, -0.29f, 0.22f };
		...
		/* --- Print Camera Info --- */
		{
			if (false)
			{
				...
				if (time >= 1.0f)
				{
					...
					float yaw = mCam.cam.yaw;
					float pitch = mCam.cam.pitch;

					Logger::info("Cam Pos: ({:.2f}, {:.2f}, {:.2f}) | Yaw: {:.2f} | Pitch: {:.2f}",
